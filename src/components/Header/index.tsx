...
import './index.scss'
import { ProfileProps } from "../../api/profileFetch"
import { Link, useLocation } from "react-router-dom"
import { giveProfile } from "../../api/selectors"
import { ProfileLink, Search } from ".."

export interface profilePropsItem {
    ...
    return (
        <header className="header">
            <div className="header__container container">
                <Link to={"/"} className="header__logo" tabIndex={-1}>
                    <img src="/logo.svg" alt="логотип" />
                </Link>
                <nav className="header__nav">
                    <ul className="header__nav__list">
                        <li className={`header__nav__list__item${pathname === '/' ? ' active' : ''}`}>
                            <Link to={'/'} className="header__nav__link link-main nav-link-def">Главная</Link>
                        </li>
                        <li className={`header__nav__list__item${pathname.includes('/genres') ? ' active' : ''}`}>
                            <Link to={'/genres'} className="header__nav__link link-genre nav-link-def">
                                <span className="link-genre__span">Жанры</span>
                                <svg className="link-genre__svg" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <li className="header__nav__list__item item-search">
                            <Search />
                        </li>
                        <li className="header__nav__list__item">
                            {profile ? <ProfileLink name={profile.name} /> : <LoginBtn />}
                        </li>
                    </ul>
                </nav>
            </div>
