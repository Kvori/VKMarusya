...
export const ProfileLink = ({ name }: ProfileBtnProps) => {
    const nameString = name.toLocaleUpperCase().slice(0, 1) + name.toLocaleLowerCase().slice(1)
    return (
        <Link to={"/profile"} className="header__nav__link login-btn nav-link-def">
            <span className="profile-desktop">{nameString}</span>
