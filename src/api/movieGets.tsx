...
export const getMovieList = (): Promise<MovieList> => fetch(`${BASE_URL}/movie`).then(res => res.json());

export const getMovieListGenre = (genre: string, page: number): Promise<MovieList> => fetch(`${BASE_URL}/movie?genre=${genre}&count=10&page=${page}`)
.then(validateResponse)
.then(res => res.json());

export const getMovieListTitle = (title: string): Promise<MovieList> => fetch(`${BASE_URL}/movie?title=${title}&count=5`)
.then(validateResponse)
.then(res => res.json())
.then(data => MovieListScheme.parse(data))

export const getRandomMovie = (): Promise<Movie> => fetch(`${BASE_URL}/movie/random`).then(res => res.json());

export const getTopMovies = (): Promise<MovieList> => fetch(`${BASE_URL}/movie/top10`).then(res => res.json());

export const getMovie = (id: string): Promise<Movie> => fetch(`${BASE_URL}/movie/${id}`)
.then(res => res.json())
